...
class HummingbirdDynamics:
    ...
    def _M(self, state):
        ...
        M33 = (self.m1*self.ell1**2 + self.m2*self.ell2**2 + self.J2z + self.J1y*np.sin(phi)**2 + self.J1z*np.cos(phi)**2) * np.cos(theta)**2 + \
            (self.J1x + self.J2x) * np.sin(theta)**2 + self.m3*(self.ell3x**2 + self.ell3y**2) + self.J3z
    ...
    def _C(self, state):
        ...
        entry1 = (self.J1y - self.J1z) * np.sin(phi) * np.cos(phi) * (thetadot**2 - np.cos(theta)**2 * psidot**2) + \
            ((self.J1y - self.J1z) * (np.cos(phi)**2 - np.sin(phi)**2) - self.J1x) * np.cos(theta) * thetadot * psidot
        entry2 = 2*(self.J1z - self.J1y) * np.sin(phi) * np.cos(phi) * phidot * thetadot + \
            ((self.J1y - self.J1z) * (np.cos(phi)**2 - np.sin(phi)**2) + self.J1x) * np.cos(theta) * phidot * psidot - \
            1/2 * N33 * psidot**2
        entry3 = thetadot**2 * (self.J1z - self.J1y) * np.sin(phi) * np.cos(phi) * np.sin(theta) + \
            ((self.J1y - self.J1z) * (np.cos(phi)**2 - np.sin(phi)**2) - self.J1x) * np.cos(theta) * phidot * thetadot + \
            (self.J1z - self.J1y) * np.sin(phi) * np.cos(phi) * np.sin(theta) * thetadot**2 + \
            2*(self.J1y - self.J1z) * np.sin(phi) * np.cos(phi) * phidot * psidot + \
            2 * (-self.m1 * self.ell1**2 - self.m2 * self.ell2**2 - self.J2z + self.J1x + self.J2x + self.J1y * np.sin(phi)**2 + self.J1z*np.sin(phi)**2) * \
            np.sin(theta) * np.cos(theta) * thetadot * psidot
 
        # Return the C matrix
        return np.array([[entry1],
                [entry2],
                [entry3],
                ])
